$(function(){
	...
	$('.panel-group').on('click', '.panel>.panel-heading>.delSeriesImg', function(event){
		...
		layui.use('layer', function(){
			  ...
			  layer.open({
				  content: '<div style="text-align:center">是否确定删除<font style="color:red">'+title+'</font>系列？删除不可逆!</div>'
				  ,btn: ['确定', '取消']
			      //按钮位置
			  	  ,btnAlign: 'c'
			  	  //关闭按钮格式
			...
			success : function(data) {
				...
				for(var i = 0 ; i < data.length ; i++){
					...
					if(course.length>0){
						...
						table="<font class='noCourse'>该系列暂无课程</font>";
					}
